Rails.application.routes.draw do
  ...
  root :to => 'pages#index' #rootをpages/indexに変更した ※/→#にする
  ...
  resources :users, only:[:show] #usersのルートのshowのみできる
  
  resources :photos, only: [:create, :destroy] do 
    collection do
      get :list
    end 
  end #onlyで、photoは、createとdestroyのみできる。collection
  ...
  get 'manage-listing/:id/caledar' => 'listings#calendar', as:'manage_listing_calendar'
